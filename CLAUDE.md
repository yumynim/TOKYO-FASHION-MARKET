...
- **画像は必ずWeb用に圧縮してから `img/` に置く**（**WebP形式・品質75前後を第一候補**とする。
  JPEGより4〜5割軽く全ての現行ブラウザで表示できる。1枚100KB以下目安・長辺1600px程度まで縮小・
  `-strip`でメタデータ除去）。元の高解像度データは `資料/` に保管する。
  ファーストビューの大きい画像はスマホ用に横800px版（`-sm`サフィックス）も作り、
  CSSメディアクエリ＋preloadのmedia属性で出し分ける（`img/page-head/` が実例）。
  商品が増えるほどページの重さに直結するため厳守
- **一覧に並ぶ `<img>` には `loading="lazy" decoding="async"` を付け、
  CSSで縦横比の枠（`aspect-ratio`等）を確保する**（読み込みで画面がガタつくのを防ぐ。
  ファーストビューの1枚だけは `<link rel="preload" as="image">` で先行読み込みする）
